...
cells_01 = [
    ...
    code_cell([
        "# DEINE LÖSUNG HIER:",
        "top10_result = await call_tool(\"simulate_tournament\", {\"n_sims\": 1000})",
...
cells_02 = [
    ...
    code_cell([
        ...
        "MCP_URL = \"https://wm2026.fh-swf.cloud/mcp\"",
        "",
        "async def get_simulation(n_sims):",
        "    async with streamablehttp_client(MCP_URL) as (read_stream, write_stream):",
        "        async with ClientSession(read_stream, write_stream) as session:",
        "            await session.initialize()",
        "            result = await session.call_tool(\"simulate_tournament\", arguments={\"team\": \"Deutschland\", \"n_sims\": n_sims})",
        "            if not result.content:",
        "                return {}",
        "            return json.loads(result.content[0].text)",
        "",
        "print(\"=== 10 Simulationen ===\")",
        "sim10 = await get_simulation(10)",
        "print(sim10.get(\"summary\"))",
        "print(\"\\n=== 5000 Simulationen ===\")",
        "sim5000 = await get_simulation(5000)",
        "print(sim5000.get(\"summary\"))"
    ]),
    markdown_cell([
        "### 🧠 Aufgabe für dich:",
    ...
    code_cell([
        "# DEINE LÖSUNG HIER:",
        "async def run_usa_simulation(home_boost):",
        "    async with streamablehttp_client(MCP_URL) as (read_stream, write_stream):",
        "        async with ClientSession(read_stream, write_stream) as session:",
        "            await session.initialize()",
        "            res = await session.call_tool(\"simulate_tournament\", arguments={\"team\": \"USA\", \"n_sims\": 5000, \"home_boost\": home_boost})",
        "            return json.loads(res.content[0].text) if res.content else {}",
        "",
        "usa_no_boost = await run_usa_simulation(0)",
        "usa_with_boost = await run_usa_simulation(150)",
        "",
...
cells_04 = [
    ...
    code_cell([
        ...
        "    return \"\\n\\n\".join(docs)",
        "",
        "async def call_mcp_tool(tool_name: str, parameters: dict) -> str:",
    ...
    code_cell([
        ...
        "            else:",
        "                result_str = await call_mcp_tool(func_name, func_args)",
        "                ",
        "            messages.append({",
        "                \"role\": \"tool\",",
    ...
    code_cell([
        "agenten_antwort = await run_agent(",
